...
function ZpPlatform (log, config) {
  ...
  this.tuneInStations = (config.tuneInStations || []).map(s => {
    const name = `TuneInStation${s.name}`
    my.createCharacteristic(name, s.uuid, {
      format: my.formats.BOOL,
      perms: [my.perms.READ, my.perms.NOTIFY, my.perms.WRITE]
    }, s.displayName)

...
function ZpAccessory (platform, zp) {
  ...
  for (const station of platform.tuneInStations) {
    const {stationId, displayName, name} = station;
    this.groupService.addOptionalCharacteristic(my.Characteristic[name])
    this.groupService.getCharacteristic(my.Characteristic[name])
      .updateValue(false)
      .on('set', (on, callback) => this.onChangeTuneInStation.apply(
          this, [on, stationId, displayName, callback]
      ))
  }
        ...
        this.parser.parseString(data, (err, json) => {
          if (!err && json['DIDL-Lite']) {
            ...
            if (item.res != null && item.res[0] != null && item.res[0]._ != null) {
              ...
              const [mediaType, mediaUri] = type.split(':');
              switch (mediaType) {
                ...
                case 'x-sonosapi-stream': // Radio stream.
                  const [stationId, parameters] = mediaUri.split('?')
                  this.updateTuneInCharacteristics(stationId.substr(1))
...
ZpAccessory.prototype.updateTuneInCharacteristics = function (currentStationId) {
  this.log.debug('%s: updateTuneInCharacteristics', this.name)

  for (const station of this.platform.tuneInStations) {
    const stationIsOn = station.stationId === currentStationId

    this.log.info('%s: Tunein Radio station %s is now %s', this.name, station.displayName, stationIsOn ? 'on' : 'off')
    this.groupService.updateCharacteristic(my.Characteristic[station.name], stationIsOn)
  }
}

ZpAccessory.prototype.onChangeTuneInStation = function(on, stationId, displayName, callback) {
  this.log.debug('%s: onChangeTuneInStation', this.name)
  const onOff = !!on;

  if (onOff) {
    this.log.debug('%s: playTuneinRadio', this.name)
    this.zp.playTuneinRadio(stationId, displayName, (err) => {
      if (err) {
        this.log.error('%s: playTuneinRadio: %s', this.name, err)
        return callback(err)
      } else {
        this.log.info('%s: Plays Tunein Radio Station %s (%s) now', this.name, displayName, stationId)
        callback()
      }
    })
  } else {
